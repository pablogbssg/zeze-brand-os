...
const NAV = [
  ...
  { id:'techpack',  label:'Tech Pack',    color:'#6EE7B7' },
]

export default function Topbar({ page, onChange, theme, onToggleTheme, onLogout }) {
  const now = new Date()
    ...
    <div style={{ display:'flex', alignItems:'center', padding:'0 28px', height:'56px', background:'var(--bg2)', borderBottom:'1px solid var(--border)', gap:'24px', position:'sticky', top:0, zIndex:20 }}>
      <div style={{ display:'flex', alignItems:'center', gap:'8px', flexShrink:0 }}>
        <svg width="52" height="18" viewBox="0 0 180 55">
          <defs><linearGradient id="tlg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#A78BFA"/><stop offset="100%" stopColor="#F472B6"/></linearGradient></defs>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'8px', marginLeft:'auto', flexShrink:0 }}>
        <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'20px', padding:'5px 14px', fontSize:'11px', color:'var(--text2)', fontWeight:500 }}>{dateStr}</div>
        <button onClick={onToggleTheme} style={{ width:32, height:32, borderRadius:'8px', border:'1px solid var(--border)', background:'var(--bg3)', color:'var(--text2)', fontSize:'14px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
          {theme==='dark'?'☀️':'🌙'}
        </button>
        <button onClick={onLogout} style={{ width:32, height:32, borderRadius:'50%', border:'none', background:'linear-gradient(135deg,#7C3AED,#EC4899)', color:'white', fontSize:'11px', fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }} title="Ausloggen">ZZ</button>
      </div>
    </div>
  )
}
