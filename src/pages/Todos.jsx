  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 16px', borderBottom:'1px solid #1a1a1a', transition:'.1s' }}
      ...
      <button onClick={handleSend} title="Ins Board übertragen"
        style={{ background: sent ? '#06503022' : '#1a1a1a', border:`1px solid ${sent ? '#6EE7B744' : '#2a2a2a'}`, borderRadius:7, padding:'4px 9px', fontSize:11, color: sent ? '#6EE7B7' : '#555', cursor:'pointer', flexShrink:0, transition:'.2s', whiteSpace:'nowrap' }}
        onMouseEnter={e=>{ if(!sent){ e.currentTarget.style.background='#3730a322'; e.currentTarget.style.color='#A78BFA'; e.currentTarget.style.borderColor='#A78BFA44' }}}
        onMouseLeave={e=>{ if(!sent){ e.currentTarget.style.background='#1a1a1a'; e.currentTarget.style.color='#555'; e.currentTarget.style.borderColor='#2a2a2a' }}}>
        {sent ? '✓ Gesendet' : '→ Board'}
      </button>
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:16, alignItems:'center' }}>
        ...
        <div style={{ marginLeft:'auto', fontSize:11, color:'#444' }}>→ Board überträgt die Task ins Kanban Board</div>
      </div>
